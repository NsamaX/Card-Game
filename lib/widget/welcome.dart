...
class WelcomeWidget {
  ...
  Widget image(String image, double size) {
    return Image.asset(image,
        width: size, height: size, color: theme.iconTheme.color);
  }
  ...
  Widget signOption(Map<String, String> image, double size) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 60),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children:
            image.entries.map((entry) => option(entry.value, size)).toList(),
      ),
    );
  }

  Widget option(String image, double size) {
    return Padding(
